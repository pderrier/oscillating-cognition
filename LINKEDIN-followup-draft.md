...
*Follow-up to [Another Approach to Building Machine Intelligence: Between Chaos and Structure](https://www.linkedin.com/pulse/another-approach-building-machine-intelligence-between-pierre-derrier-2ow1f/)*

---

A few months ago, I published an article about [Oscillating Cognition](https://www.linkedin.com/pulse/another-approach-building-machine-intelligence-between-pierre-derrier-2ow1f/) — a system designed to make LLMs think more like humans.
...
The system runs as an [MCP server](https://github.com/pderrier/oscillating-cognition) — any AI agent can call `oscillate(seed="your topic", cycles=3)` and get back crystallized insights, open questions, and optionally grounded action proposals. It also has a CLI and a web UI.
...
We picked **10 topics across domains** that have nothing to do with our project — to prevent circular evaluation. Things like:
...
One judge noted: *"The overlap is so extensive that it suggests a shared underlying template rather than independent reasoning."*
...
## Track 2: Stripping the Grounding — What Does Oscillation Actually Think?
...
|---|:-:|:-:|:-:|
| Novelty | 9.0 | 7.2 | **+1.8** |
| Preserved contradictions | 9.4 | 7.7 | **+1.8** |
| Emergent insight | 9.0 | 7.5 | **+1.5** |
| Human-likeness | 7.9 | 7.2 | +0.8 |
| Depth | 8.6 | 8.0 | +0.6 |
| **Specificity** | **6.5** | **8.2** | **-1.7** |
...
A judge summarized it this way:
...
We tested this on three philosophical/societal topics — machine consciousness, life-extension inequality, and AI-generated art. Each system's raw thinking was grounded into an essay, then judged on: narrative voice, tension as structure, originality of framing, "would you keep reading?", and intellectual honesty.
...
(Full essays are available in the [eval data](https://github.com/pderrier/oscillating-cognition/blob/main/eval/generated_essays.md) if you want to judge for yourself.)
...
On the strategy seed, Opus gave these scores:

| Criterion | Oscillation | Baseline |
|---|:-:|:-:|
| Non-obviousness | 8 | 5 |
| Clarity of tradeoffs | 9 | 8 |
| Actionability | 5 | 8 |
| Decision-readiness | 6 | 8 |

The total scores are nearly tied (7.0 vs 7.0). But **which criteria you weight determines who wins**. GPT implicitly weighted actionability and decision-readiness. Opus implicitly weighted originality and tradeoff clarity.
...
This has implications far beyond our project. **Anyone using LLM-as-judge should know that a single model gives you one implicit value system, not ground truth.** Cross-model judging doesn't just reduce bias — it surfaces tensions in what "quality" means. (We wrote a [standalone analysis](https://github.com/pderrier/oscillating-cognition/blob/main/eval/MODEL-DIVERGENCE.md) on this finding.)
...
The pattern holds across every mode:

| Use case | Recommended | What the judges said |
|----------|:-----------:|-----|
| Essay / thought piece | **Oscillation** | *"Tensions destabilize the reader's footing"* vs *"tensions organize the reader's learning."* Delta: +3.0 on originality of framing. |
| Workshop provocations | **Oscillation** | *"Harder to deflect with stock answers."* Oscillation's questions make people pause mid-argument. |
| Research ideation | **Oscillation** | *"Would change how we interpret the debate itself"* vs baseline that *"confirms existing intuitions."* |
| Research protocols | **Baseline** | Falsifiability +3.0. *"Could be handed to a modeler tomorrow."* |
| Strategy (explore) | **Oscillation** | *"Every option is built around a real learning-science insight."* Sharper tradeoffs, more non-obvious. |
| Strategy (decide) | **Baseline** | *"Better portfolio coverage."* Spans a risk spectrum for an exec team. |
| Policy memos | **Either** | Generic grounding makes both identical — the formatting erases the difference. |
...
**[github.com/pderrier/oscillating-cognition](https://github.com/pderrier/oscillating-cognition)**

---

*The evaluation was run using GPT-5.4 (via Codex OAuth) for generation and judging, with Claude Opus 4.6 as cross-model judge. Full methodology in [PROTOCOL.md](https://github.com/pderrier/oscillating-cognition/blob/main/eval/PROTOCOL.md). Complete results with all judge quotes in [RESULTS.md](https://github.com/pderrier/oscillating-cognition/blob/main/eval/RESULTS.md). Model divergence analysis in [MODEL-DIVERGENCE.md](https://github.com/pderrier/oscillating-cognition/blob/main/eval/MODEL-DIVERGENCE.md).*
